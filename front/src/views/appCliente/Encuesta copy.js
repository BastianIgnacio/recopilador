...
const Encuesta = ({ match, id, grupo, ws, bloqueado }) => {
  ...
  const generarPersonasDependenOptions = (min, max) => {
    ...
    // eslint-disable-next-line no-plusplus
    for (let i = min; i < max; i++) {
      array.push({ value: i, label: `${i} persona` });
    }
    return array;
  };
              ...
              >
                {({
                  ...
                  <Form className="av-tooltip">
                    ...
                    <FormGroup row className="m-3 tooltip-right-bottom">
                      <Label sm={4}>¿Cual es su comuna de residencia?</Label>
                      <Colxx sm={4}>
                        <select
                          disabled={botonDisabled}
                          name="selectRegion"
                    ...
                    <FormGroup row className="m-3 tooltip-right-bottom">
                      <Label sm={4}>
                        ¿Cuantas personas que viven con usted dependen
                        economicamente de los ingresos que usted aporta al hogar
                        ?
                      </Label>
                      <Colxx sm={8}>
                        <select
                        ...
                        >
                          ...
                          {generarPersonasDependenOptions(1, 15).map((opc) => (
                            <option key={opc.value} value={opc.value}>
                              {opc.label}
                            </option>
                          ))}
                    ...
                    <FormGroup row className="m-3 tooltip-right-bottom">
                      <Label sm={4}>
                        ¿Cuantos años lleva usted viviendo en este lugar ?
                      </Label>
                      <Colxx sm={8}>
                        <select
                          disabled={botonDisabled}
                        ...
                        >
                          ...
                          {generarEdadOptions(1, 60).map((opc) => (
                            <option key={opc.value} value={opc.value}>
                              {opc.label}
                            </option>
                          ))}
